...
class Point(object):
    ...
    def move_to(self,x, y):
        ...
        self.y = y
        self.moves = self.moves + 1
        self.travDist += ((self.x - x)**2+(self.y - y)**2)**(1/2)
    ...
    def get_distance_from_start(self):
        return ((self.x - self.initX) ** 2 + (self.y - self.initY) ** 2) **(1/2)

    def get_distance_traveled(self):
        return self.travDist

    def closer_to(self, p1, p2):
        if( ((self.x - p1.x)**2+(self.y - p1.y)**2)**(1/2) > ((self.x - p2.x)**2+(self.y - p2.y)**2)**(1/2) ):
            return p2
        else:
            return p1

    def halfway_to(self,oP):
        return Point((self.x+oP.x)/2, (self.y + oP.y) / 2)
